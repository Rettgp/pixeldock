...
Given("there are multiple appmanifest files with different StateFlags", function () {
  mockFs({
...
Then("only games with StateFlag {string} are included", function (stateFlag: string) {
...
Then("the games should include:", function (dataTable) {
  ...
  for (const expected of expectedGames) {
    const match = result.find(g => g.appid === expected.appid && g.name === expected.name);
